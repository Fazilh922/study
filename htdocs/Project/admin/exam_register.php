...
<body class="ttr-opened-sidebar ttr-pinned-sidebar"> 
	...
	<main class="ttr-wrapper">
		...
		<form class="edit-profile m-b30" method="post" autocomplete="off" action="action/exam_register.php" enctype="multipart/form-data">
								<div class="row">
									<div class="col-12">
										<div class="ml-auto">
										...
										<div>
                                            ...
                                            <datalist id="name">
                                                <option value="">Select</option> 
                                                 <?php
                                                    $result = $db->prepare("select distinct sname from user_post where Log_Id='$Log_Id'");
                                                    $result->execute();
                                    ...
                                    <div class="form-group col-4">
										...
										<div>
                                            ...
                                            <datalist id="deprt">
                                                <option value="">Select</option> 
                                                 <?php
                                                    $result = $db->prepare("select distinct deprt from user_post where Log_Id='$Log_Id'");
                                                    $result->execute();
                                                    for($i=0; $rows = $result->fetch(); $i++)
                                                    {
                                                    echo '<option>'.$rows['deprt'].'</option>';
                                                    }
                                                ?>	                                         					
                                            </datalist>                                        	
										</div>
                                    ...
                                    <div class="form-group col-4">
										...
										<div>
                                            ...
                                            <datalist id="semstr">
                                                <option value="">Select</option> 
                                                 <?php
                                                    $result = $db->prepare("select distinct semstr from user_post where Log_Id='$Log_Id'");
                                                    $result->execute();
                                                    for($i=0; $rows = $result->fetch(); $i++)
                                                    {
                                                    echo '<option>'.$rows['semstr'].'</option>';
                                                    }
                                                ?>	                                         					
                                            </datalist>                                        	
										</div>
